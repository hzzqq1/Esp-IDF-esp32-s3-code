...
void timg_init(timg_config_t *timg_config)
{
  ...
  esp_clk_tree_src_get_freq_hz(timg_config_clk->clk.src, ESP_CLK_TREE_SRC_FREQ_PRECISION_CACHED, clk_src_hz);

  timer_config.alarm_en = TIMER_ALARM_EN;
  timer_config.auto_reload = timg_config->auto_reload;
  timer_config.clk.src = timg_config->clk_src;
  ...
  ESP_ERROR_CHECK(timer_isr_callback_add(timg_config->timer_group, timg_config->timer_idx, IRAM_ATTR timer_group_isr_callback, timg_config, 0));
